    ...
    def getAction(self, gameState):
        ...
        def maximizer(state, depth, index_of_agent):
            ...
            for action in Actions:
                ...
                newValue=minimizer(newState,depth,1)[0]
                if newValue>value:
                    value=newValue
                    maxiAction=action
        ...
        def minimizer(state, depth, index_of_agent):
            ...
            #start
            #需要枚举每个智能体的所有行为:通过index_of_agent来进行

            if( len(state.getLegalActions(index_of_agent))==0 ):
                return ( self.evaluationFunction(state),None )
            ...
            for action in state.getLegalActions(index_of_agent):
                ...
                elif index_of_agent==state.getNumAgents()-1:
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def maximizer(state, depth, index_of_agent, alpha, beta):
            ...
            #condition for termination of recursive method calls
            def terminal_condition(state,depth):
                return state.isWin() or state.isLose() or depth==0 or len( state.getLegalActions(index_of_agent) )==0
            ...
            for action in state.getLegalActions(0):
                newState=state.generateSuccessor(0,action)
                newValue=minimizer(newState,depth,1,alpha,beta)[0]
                if newValue > value:
                    value=newValue
                    maxiAction=action
                if value > beta:
    ...
    def getAction(self, gameState, mcts_time_limit=10):
        ...
        def Selection(cgs, cgstree):
            '''
                ...
                cgstree: current game state tree, <class 'Node'>  more precisely, a partial branch
            ...
            for i in range(n):
                ...
                if action=="North" and cgstree.north==None:
                    unexplored=True
                if action=="East" and cgstree.east==None:
                    unexplored=True
                if action=="West" and cgstree.west==None:
                    unexplored=True
                if action=="South" and cgstree.south==None:
                    unexplored=True
                if action=="Stop" and cgstree.stop==None:
                    unexplored=True
            ...
            best_child=best_UCT(children)[0]

            cgs=best_child
            for child, action in children:
                if child.statevalue == cgs:
                    cgstree = child
        ...
        def best_UCT(children, random_prob=0.3):
            '''
                children: list of tuples, each tuple contains a child node(the class Node defined ahead) and the action that leads to it
