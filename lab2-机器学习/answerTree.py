...
def gain(X: np.ndarray, Y: np.ndarray, idx: int):
    ...
    H_D=entropy(Y)
    H_A=entropy(feat)
    Z= np.column_stack([Y, feat])
    values,counts=np.unique(Z,axis=0,return_counts=True)
    probs=counts/counts.sum()
    H_D_and_A=-np.sum(probs*np.log2(probs))

    return H_D+H_A-H_D_and_A
...
def buildTree(X: np.ndarray, Y: np.ndarray, unused: List[int], depth: int, purity_bound: float, gainfunc: Callable, prefixstr=""):
    ...
    # 当达到终止条件时，返回叶节点
    # TODO: YOUR CODE HERE
    if entropy(Y) <= purity_bound or len(unused) == 0 or depth <= 0:
        return root
    ...
    for i in ufeat:
        ...
        childX = X[filter]
        childY = Y[filter]
        root.children[i]=buildTree(childX,childY,unused,depth-1,purity_bound,gainfunc)
    return root
